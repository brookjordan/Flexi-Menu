...
MU_System.factory("muMenus", [ "$rootScope", "$timeout", function($rootScope) {
    ...
    function resizeHandler() {
        var resizeIntervalLimit = 333, timeNow = +new Date();
    ...
    function runResize() {
        findContentSize(), setContentQueryString(), setContainerQueryString(), $rootScope.$broadcast("MU_windowResized"), 
        lastResize = +new Date();
    }
    ...
    function addQuery(options) {
        for (var j, arg, queries, query, i = 1, args = arguments, o = options || {}, type = "string" == typeof o ? o : o.type, callback = o.callback || void 0; i < args.length; i += 1) {
            ...
            for (j = 0; j < queries.length; j += 1) query = queries[j], contextQueries[type].push(+query), 
            queryCallbacks[query] = queryCallbacks[query] || [], callback && queryCallbacks[query].push(callback);
        }
        return contextQueries[type].sort(sortNumber), setContentQueryString(), setContainerQueryString(), 
        $rootScope.$broadcast("MU_windowResized"), muContent;
    }
    function setContentQueryString() {
        for (var query, greaterThan = "", i = 0; i < contextQueries.w.length && (query = contextQueries.w[i], 
        !(query > muContent.contentWidth)); i += 1) greaterThan += " media__min-width__" + query;
        muContent.contentClass = greaterThan;
    }
    function setContainerQueryString() {
        for (var query, prevQuery, greaterThan = "", i = 0; i < contextQueries.w.length && (prevQuery = query, 
        query = contextQueries.w[i], !(query > muContent.containerWidth)); i += 1) greaterThan += " media__min-width__" + query;
        muContent.containerlass = greaterThan, prevQuery && runQueryCallbacks(prevQuery);
    }
    function runQueryCallbacks(querySize) {
        var callback, i = 0, callbacks = queryCallbacks[querySize];
    ...
    function muContent() {}
    var systemMetrics = {}, menus = muMenus.menusBase, contextQueries = {
        w: [],
        h: []
    }, queryCallbacks = {}, delayBroadcastResize = !1, lastResize = +new Date();
    ...
    }, muContent.contentClass = "", muContent.containerlass = "", muContent.containerWidth = 0, 
...
angular.module("myApp").run([ "$templateCache", function($templateCache) {
    ...
    self.menus = muMenus, self.content = muContent, self.leftIcons = "0 1 2 3 4".split(" "), 
